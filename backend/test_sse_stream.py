    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                print(f"✅ Created test investigation: {investigation['id']}")
                    ...
                    async for line in response.aiter_lines():
                        if line.startswith("data: "):
                            ...
                            try:
                                ...
                                print(f"   Investigation: {event.get('investigation_id')}")
                                ...
                                if event.get('ui_summary'):
                                    print(f"   Summary: {event.get('ui_summary')[:100]}...")
                                
                                print(f"   Timestamp: {event.get('timestamp')}")
                                print("-" * 40)

                                # Stop after getting some events or on completion
                                if event.get("type") == "completion" or event_count >= 5:
